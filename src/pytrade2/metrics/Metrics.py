...
from pytrade2.metrics.MetricsBase import MetricsBase
...
class Metrics(MetricsBase):
    ...
    class Strategy:
            ...
            class Candles:
                def __init__(self, app_name: str, strategy: str):
                    self.open = Gauge("strategy_feed_candles_open",
                                      "Candles", namespace=app_name, subsystem=strategy, registry=MetricsBase.registry,  labelnames=["strategy"]).labels(strategy = MetricsBase.strategy)
        ...
        class Learn:
            def __init__(self, app_name: str, strategy: str):
                self.train_period_sec = Gauge("strategy_learn_train_period_sec",
                                              "History amount to use for train", namespace=app_name, subsystem=strategy, registry=MetricsBase.registry,  labelnames=["strategy"]).labels(strategy = MetricsBase.strategy)
                self.train_exec_duration_sec = Gauge("strategy_learn_train_exec_duration_sec",
                                                     "Model fit() process duration", namespace=app_name,
                                                     subsystem=strategy, registry=MetricsBase.registry,  labelnames=["strategy"]).labels(strategy = MetricsBase.strategy)
        ...
        class Signal:
            def __init__(self, app_name: str, strategy: str):
                self.signal = Gauge("strategy_signal", "strategy_signal", namespace=app_name, subsystem=strategy, registry=MetricsBase.registry,  labelnames=["strategy"]).labels(strategy = MetricsBase.strategy)
        ...
        class Order:
            def __init__(self, app_name: str, strategy: str):
                ...
                self.order_create_not_filled = Gauge("broker_order_create_not_filled", "order_create_not_filled",
                                                     namespace=app_name, subsystem=strategy, registry=MetricsBase.registry,  labelnames=["strategy"]).labels(strategy = MetricsBase.strategy)
        ...
        class Trade:
            def __init__(self, app_name: str, strategy: str):
                ...
                self.trade_close_price = Gauge("broker_trade_close_price", "trade_close_price", namespace=app_name,
                                               subsystem=strategy, registry=MetricsBase.registry,  labelnames=["strategy"]).labels(strategy = MetricsBase.strategy)
                self.trade_sl = Gauge("broker_trade_sl", "trade_sl", namespace=app_name, subsystem=strategy, registry=MetricsBase.registry,  labelnames=["strategy"]).labels(strategy = MetricsBase.strategy)
                self.trade_tp = Gauge("broker_trade_tp", "trade_tp", namespace=app_name, subsystem=strategy, registry=MetricsBase.registry,  labelnames=["strategy"]).labels(strategy = MetricsBase.strategy)
