...
class TestLowHighTargets(TestCase):
    def test_fut_lohi(self):
        candles = pd.DataFrame([
            ...
            {'datetime': datetime.fromisoformat('2024-02-07 20:03'), 'low': -4, 'high': 4},
            {'datetime': datetime.fromisoformat('2024-02-07 20:04'), 'low': -7, 'high': 7},
        ]).set_index('datetime')

        actual = LowHighTargets.fut_lohi(candles, '1min')
        self.assertSequenceEqual(candles.index[:-1].tolist(), actual.index.tolist())
        self.assertSequenceEqual([-1, -2, -3], actual['fut_low_diff'].tolist() )
        self.assertSequenceEqual([1,2,3], actual['fut_high_diff'].tolist() )
