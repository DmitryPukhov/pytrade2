...
class TestLSTMStrategyBase(TestCase):
    def test_reshape_func_123_2(self):
        x = np.array([[1, 1, 1], [2, 2, 2], [3, 3, 3]])
        actual = LSTMStrategyBase.reshape_x(x, window_shape=(2, 3))
    ...
    def test_reshape_func_1_1(self):
        x = np.array([[1, 1, 1], [2, 2, 2], [3, 3, 3], [4, 4, 4]])
        actual = LSTMStrategyBase.reshape_x(x, window_shape=(1, 3))
        self.assertListEqual([[[1, 1, 1]], [[2, 2, 2]], [[3, 3, 3]], [[4, 4, 4]]], actual.tolist())

    def test_reshape_func_1_2(self):
        x = np.array([[1, 1, 1]])
        actual = LSTMStrategyBase.reshape_x(x, window_shape=(10, 3))
        self.assertListEqual([], actual.tolist())
